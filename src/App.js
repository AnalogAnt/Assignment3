...
class App extends Component {
  ...
  onClickThumbnail = id => {
    const {matchId, time} = this.state
    const randomNumber = parseInt((Math.random() * 100) % 30)
    if (id === imagesList[matchId].id && time > 0) {
  ...
  render() {
    const {score, time, gameResult, currentCategory, matchId} = this.state
    const filteredThumbanilList = imagesList.filter(
      each => each.category === currentCategory,
    )
    const matchImage = (
      <img
        src={imagesList[matchId].imageUrl}
        alt="match"
        className="matchImage"
      />
    )
    let LowerCon
    if (gameResult === 'Running' && time > 0) {
      LowerCon = (
        <div className="runingCon">
          {matchImage}
          <ul className="tabsCon">
            {tabsList.map(each => (
              <TabItem
                key={each.tabId}
                details={each}
                isTabOn={each.tabId === currentCategory}
                onTabChange={this.onTabChange}
              />
            ))}
          </ul>
          <ul className="thumbnailCon">
            {filteredThumbanilList.map(each => (
              <ThumbnailItem
                key={each.id}
                details={each}
      ...
      LowerCon = (
        <div className="ResultCon">
          ...
          <button className="butt" onClick={this.onReset} type="button">
            <img
    ...
    return (
      <div className="bg-con">
        <ul className="navbar">
          ...
          <li className="scoreCon">
            <p className="scoreParaa">
        ...
        <div className="lowerPart">{LowerCon}</div>
      </div>
    )
  }
}
